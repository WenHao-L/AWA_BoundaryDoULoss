...
from segmentation.aw_module import AW_Module
from segmentation.WSO_PyTorch import WindowOptimizer
from segmentation.aw_swinunetr import AW_SwinUNETR
from segmentation.aw_unetr import AW_UNETR
from segmentation.swinunetr import SwinUNETR
...
class AW_UNet(nn.Module):
    def __init__(self, stem, down, up, tail, width, conv_builder, n_channels=1, n_classes=2, dropout_flag=True, use_checkpoint=False):
        super(AW_UNet, self).__init__()

        self.aw_module = AW_Module(in_ch=3, use_checkpoint=use_checkpoint)
...
class AW_ITUNet_2d(nn.Module):
    def __init__(self, stem, down, up, tail, width, conv_builder,image_size = 128, transformer_depth = 18, n_channels=1, n_classes=2, dropout_flag=True, use_checkpoint=False):
        super(AW_ITUNet_2d, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.width = width
        self.dropout_flag = dropout_flag
    ...
    def forward(self, x):
        if self.n_channels == 5:
            x_aw = x[:, :3]
            x_aw = self.aw_module(x_aw)
            x = torch.cat((x_aw, x[:, -2:]), dim=1)
        else:
            x = self.aw_module(x)
        ...
        # return logits
        return [logits, out1, out2, out3, out4]
        # return [logits, out1, out2, out3, out4, x_aw]


class WSO_ITUNet_2d(nn.Module):
    def __init__(self, stem, down, up, tail, width, conv_builder, image_size = 128, transformer_depth = 18, n_channels=1, n_classes=2, dropout_flag=True,
                 act_window="relu", upbound_window=1., window_init=None):
        super(WSO_ITUNet_2d, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.width = width
        self.dropout_flag = dropout_flag
        ...
        self.wso_module1 = WindowOptimizer(out_channels=1, act_window=act_window, upbound_window=upbound_window, window_init=window_init, mode="2D")
        self.wso_module2 = WindowOptimizer(out_channels=1, act_window=act_window, upbound_window=upbound_window, window_init=window_init, mode="2D")
        self.wso_module3 = WindowOptimizer(out_channels=1, act_window=act_window, upbound_window=upbound_window, window_init=window_init, mode="2D")
    ...
    def forward(self, x):
        x_wso_1 = self.wso_module1(x[:, 0].unsqueeze(1))
        x_wso_2 = self.wso_module2(x[:, 1].unsqueeze(1))
        x_wso_3 = self.wso_module3(x[:, 2].unsqueeze(1))
        x_wso = torch.cat([x_wso_1, x_wso_2, x_wso_3], dim=1)
        if self.n_channels == 5:
            x = torch.cat((x_wso, x[:, -2:]), dim=1)
        else:
            x = x_wso
...
def aw_itunet_2d(**kwargs):
    return AW_ITUNet_2d(stem=DoubleConv2D,
                down=Down2D,
                up=Up2D,
                tail=Tail2D,
                width=[32,64,128,256,512],
...
def wso_itunet_2d(**kwargs):
    return WSO_ITUNet_2d(stem=DoubleConv2D,
                down=Down2D,
                up=Up2D,
                tail=Tail2D,
                width=[32,64,128,256,512],
...
def swinunetr_2d(**kwargs):
    return SwinUNETR(img_size=(384,384), in_channels=3, out_channels=2, use_checkpoint=True, spatial_dims=2)


def aw_swinunetr_2d(**kwargs):
    return AW_SwinUNETR(img_size=(384,384), in_channels=3, out_channels=2, use_checkpoint=True, spatial_dims=2)


def unetr_2d(**kwargs):
    return UNETR(in_channels=3, out_channels=2, img_size=384, feature_size=16, norm_name='batch', spatial_dims=2)


def aw_unetr_2d(**kwargs):
    return AW_UNETR(in_channels=3, out_channels=2, img_size=384, feature_size=16, norm_name='batch', spatial_dims=2)
...
def aw_unet_2d(**kwargs):
    return AW_UNet(stem=DoubleConv2D,
                down=Down2D,
                up=Up2D,
                tail=Tail2D,
                width=[32,64,128,256,512],
